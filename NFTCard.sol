...
contract CardNFT is ERC721, AccessControl {
    ...
    // INSPECTOR_ROLE = quien puede certificar/cambiar estado “oficialmente”

    // Estado (condición) de la carta: usa enum para ahorrar gas
    enum Estado {
        UNKNOWN,   // 0
        POOR,      // 1
        PLAYED,    // 2
        ...
        NEAR_MINT, // 4
        MINT,      // 5
        GRADED     // 6 (si usas grading externo)
    }
    ...
    struct CardData {
        ...
        string rareza;
        Estado estado;
        uint64 updatedAt;  // timestamp del último cambio de estado (barato)
    }
    //mapea id y carta
    mapping(uint256 => CardData) private _cardData;

    uint256 public nextTokenId = 1;
    //Creaccion de la carta logger para luego conectar con un front
    event CardMinted(
    ...
    //cambiod e estado y la fecha 
    event EstadoUpdated(
        uint256 indexed tokenId,
        address indexed operator,   // quien ejecuta el cambio
    ...
    constructor() ERC721("CardNFT", "CNFT") {
        ...
        _grantRole(INSPECTOR_ROLE, msg.sender); // puedes quitarlo si no quieres que el deployer sea inspector
    }
 ...
 * - Puede cambiarlo el OWNER
 * - O un INSPECTOR
 */
function updateEstado(uint256 tokenId, Estado nuevoEstado) external {
    address owner = ownerOf(tokenId); // revierte si no existe
...
// (Opcional) Admin puede forzar cambios
